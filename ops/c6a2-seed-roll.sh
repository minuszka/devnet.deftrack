#!/bin/bash
#
# The C6a end-height move (#244, end 13488) on the explorer VPS: the
# seed and devnet2, the canary before the fleet. Runs ON THE VPS as root.
#
# #243 is a devnet consensus change at height 13488: llmq_50_60 and llmq_60_75
# form no new quorum from there. Every devnet daemon must run this binary before
# 13488; an old one past it forks off. No conf key changes: the #231 key
...
#
#   ./c6a2-seed-roll.sh --check <seed-defcond> <fleet-defcond>
#   ./c6a2-seed-roll.sh         <seed-defcond> <fleet-defcond>
#
# The explorer reads the seed's RPC, so its sync ticks fail for the few seconds
# this takes and recover on their own.
set -uo pipefail

NEW_SEED=a7822e1b912f232fd2c5e85b90023afa
NEW_FLEET=14583e7c2b40e8de2afb265d5bc1dfea
OLD_SEED=d050b97df48a74f57a09dadca5985c84
OLD_FLEET=7bf411b0eb08731b6e050868b1d2ff72
KEYLINE='dslcommitmentv2height=12744'
LOGLINE='Setting dslcommitmentv2height to 12744'

CHECK_ONLY=0
if [ "${1:-}" = "--check" ]; then CHECK_ONLY=1; shift; fi
SEED_BIN=${1:?usage: c6a2-seed-roll.sh [--check] <seed-defcond> <fleet-defcond>}
FLEET_BIN=${2:?usage: c6a2-seed-roll.sh [--check] <seed-defcond> <fleet-defcond>}
