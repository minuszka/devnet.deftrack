#!/bin/bash
#
# The C2 roll (#235-#240, merged v22.1.x dc0db600e4) on the explorer VPS: the
...
# another name -- and every roll before #231 missed it. Both keep the #231 key
# (dslcommitmentv2height=12744) in their confs; this roll moves only the binary.
#
#   ./c2-seed-roll.sh --check <seed-defcond> <fleet-defcond>
#   ./c2-seed-roll.sh         <seed-defcond> <fleet-defcond>
#
# The explorer reads the seed's RPC, so its sync ticks fail for the few seconds
# this takes and recover on their own.
set -uo pipefail

# The canary already ran the first C2 build (38efbda119, #239) and moves on to
# the merged #240 tip; the OLD values are that first build, not the #231 one.
NEW_SEED=5c8fab67a6dcbb4c47e92e19141dcf8f
NEW_FLEET=c9898910b40a57d253dae73905e14845
OLD_SEED=6987a13b7ddfa608bbf63966515e52a2
OLD_FLEET=d555bdaa207d38f5bd3232afba21cad9
KEYLINE='dslcommitmentv2height=12744'
LOGLINE='Setting dslcommitmentv2height to 12744'

CHECK_ONLY=0
if [ "${1:-}" = "--check" ]; then CHECK_ONLY=1; shift; fi
SEED_BIN=${1:?usage: c2-seed-roll.sh [--check] <seed-defcond> <fleet-defcond>}
FLEET_BIN=${2:?usage: c2-seed-roll.sh [--check] <seed-defcond> <fleet-defcond>}
