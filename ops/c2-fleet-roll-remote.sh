#!/bin/sh
#
# Runs ON ONE FLEET HOST, copied there by ops/c2-fleet-roll.sh.
#
#   usage: c2-fleet-roll-remote.sh <staged-binary> <new-md5> <old-md5> preflight|apply
#
# The C2 roll changes the binary and nothing else: no conf key, no activation
# height. The #231 key (dslcommitmentv2height=12744) must STAY in every conf --
# a daemon started without it would demand format v2 from the first commitment
# and reject history -- so the preflight refuses a host where any conf lacks it.
#
