#!/bin/bash
#
# The C6a roll (#241-#243, merged v22.1.x ddf2822011) on the 16 fleet hosts.
# Runs ON THE JUMP HOST, after c6a-fleet-stage.sh has put the binary on every
# host. The binary moves and nothing else: no conf key. #243 activates by itself
# at height 13536, so every daemon must be on this binary before that height.
#
...
# (c6a-fleet-roll-remote.sh). The roll STOPS at the first host that does not come
# back whole -- a half-understood failure repeated on fifteen more hosts is the
# worst outcome available.
#
#   ./c6a-fleet-roll.sh --check    # preflight every host, change nothing
#   ./c6a-fleet-roll.sh            # preflight, then roll
set -uo pipefail

NEW_MD5=7bf411b0eb08731b6e050868b1d2ff72
OLD_MD5=c9898910b40a57d253dae73905e14845
STAGED=/tmp/defcond-c6a
KEY=/root/.ssh/defcon_nodes
INVENTORY=${FLEET_INVENTORY:-/root/fleet-nodes-all.txt}
HERE=$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)
REMOTE=$HERE/c6a-fleet-roll-remote.sh
...
while read -r entry; do
  ...
  echo "=== $label  $(date -u +%T)"
  $SCP "$REMOTE" "$target:/tmp/c6a-roll.sh" || { echo "  FAIL: helper copy"; failed=$((failed+1)); break; }
  pre=$($SSH "$target" "sh /tmp/c6a-roll.sh $STAGED $NEW_MD5 $OLD_MD5 preflight" 2>&1)
  echo "  $pre"
  case "$pre" in
  ...
  out=$($SSH "$target" "sh /tmp/c6a-roll.sh $STAGED $NEW_MD5 $OLD_MD5 apply; echo rc=\$?; rm -f /tmp/c6a-roll.sh" 2>&1)
