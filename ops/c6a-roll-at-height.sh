...
# multiple of 72). Runs ON THE JUMP HOST under nohup, from /root/roll-c6a.
...
# 13536, and a roll begun too close to it is a fork, not a roll.
#
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
  ...
  if [ "$h" -ge "$START" ]; then
    ...
    ./c6a-fleet-roll.sh > roll.log 2>&1
    rc=$?
