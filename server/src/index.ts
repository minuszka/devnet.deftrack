...
// x-powered-by off, the simple query parser, helmet without HSTS -- shared with
// the lab server, and before any middleware (see httpHardening.ts for why the
...
async function main(): Promise<void> {
  ...
  const simulationObservationService = new SimulationObservationService(
    ...
    new SimulationMeasurementService(new MongoSimulationMeasurementRepository()),
    {
      ...
      markUnmeasurable: (input) => simulationReconcileRepository.markMeasurementUnavailable(input),
      markReported: (input) => simulationReconcileRepository.markMeasurementReported(input),
      logger,
    }
  );
