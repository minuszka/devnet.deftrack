...
describe('operator attribution', () => {
  it('claims every masternode on a host from one line', () => {
    const idx = new OperatorIndex([
      { operatorLabel: 'op-6', proTxHashes: [], hostIps: ['178.18.247.92'] },
    ]);
    expect(idx.resolve(HASH_A, '178.18.247.92')).toBe('op-6');
    expect(idx.resolve(HASH_B, '178.18.247.92')).toBe('op-6');
  });
  ...
  it('takes the host out of a service string', () => {
    expect(hostOf('178.18.247.92:19799')).toBe('178.18.247.92');
