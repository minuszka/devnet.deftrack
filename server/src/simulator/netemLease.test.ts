...
describe('planApply', () => {
  it('applies a fault under a lease and records its expiry', () => {
    const { state, actions } = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000);
  ...
  it('is idempotent: re-applying the identical live fault does nothing', () => {
    const first = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000);
    const again = planApply(first.state, latency, RUN, 2_000, 32_000);
  ...
  it('replaces a different fault on the same container -- one qdisc per interface', () => {
    ...
    const second = planApply(first.state, loss, RUN, 2_000, 32_000);
  ...
  it('re-applies an expired lease rather than treating it as live', () => {
    const first = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000);
    const afterExpiry = planApply(first.state, latency, RUN, 40_000, 70_000);
...
describe('a lease that has already gone', () => {
  it('is refused rather than applied for a moment', () => {
    ...
    expect(ok.state.jobs[0]!.expiresAtMs).toBe(40_001);
  });
});

describe('planClear', () => {
  it('clears a known job and is idempotent for an unknown one', () => {
    const applied = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000);
...
describe('sweepExpired', () => {
  it('clears only the leases past their TTL, leaving live ones', () => {
    let state = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000).state;
    state = planApply(state, { container: 'mn02', kind: 'loss', args: ['5%'] }, RUN, 1_000, 91_000).state;
...
describe('planBootRecovery', () => {
  it('undoes every container the persisted state touched, attributed per job', () => {
    let state = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000).state;
    state = planApply(state, { container: 'mn02', kind: 'loss', args: ['5%'] }, RUN, 1_000, 31_000).state;
  ...
  it('undoes a service job with a start, not a tc delete, and does it first', () => {
    let state = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000).state;
    state = planServiceStop(state, 'mn01', RUN, 1_000, 31_000).state;
    const boot = planBootRecovery(state);
...
describe('two fault classes on one container', () => {
  it('keys state by (container, class) so neither evicts the other', () => {
    const applied = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000).state;
    const stopped = planServiceStop(applied, 'mn01', RUN, 1_000, 31_000).state;
    ...
    const reapplied = planApply(stopped, { container: 'mn01', kind: 'loss', args: ['5%'] }, RUN, 2_000, 32_000).state;
...
describe('planServiceStop', () => {
  it('is idempotent for an identical live stop and re-applies an expired one', () => {
    const first = planServiceStop(emptyWrapperState(), 'mn01', RUN, 1_000, 31_000);
    expect(first.actions).toEqual([{ op: 'stop', container: 'mn01' }]);
    const again = planServiceStop(first.state, 'mn01', RUN, 2_000, 32_000);
    expect(again.actions).toEqual([]);
    expect(again.state).toBe(first.state); // same object, as planApply does
    const expired = planServiceStop(first.state, 'mn01', RUN, 40_000, 70_000);
...
describe('undoFor', () => {
  it('derives the undo from the job class', () => {
    const netemJob = planApply(emptyWrapperState(), latency, RUN, 1_000, 31_000).state.jobs[0]!;
    expect(undoFor(netemJob)).toEqual({ op: 'clear', container: 'mn01', tcArgs: tcClearArgs() });
    const serviceJob = planServiceStop(emptyWrapperState(), 'mn01', RUN, 1_000, 31_000).state.jobs[0]!;
