...
describe('dockerExecArgv', () => {
  it('builds a docker exec tc invocation for an apply action', () => {
    ...
    expect(argv).toEqual(['exec', 'mn07', 'tc', 'qdisc', 'replace', 'dev', 'eth0', 'root', 'netem', 'delay', '100ms']);
  });

  it('builds a docker exec tc invocation for a clear action', () => {
    const argv = dockerExecArgv({ op: 'clear', container: 'mn07', tcArgs: tcClearArgs() });
    expect(argv).toEqual(['exec', 'mn07', 'tc', 'qdisc', 'del', 'dev', 'eth0', 'root']);
  });
  ...
  it('never interpolates the container into a shell string -- it is a discrete argv', () => {
    ...
    expect(argv[1]).toBe('mn01; rm -rf /');
    expect(argv).toEqual(['exec', 'mn01; rm -rf /', 'tc', 'qdisc', 'del', 'dev', 'eth0', 'root']);
  });
});
