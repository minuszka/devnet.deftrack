...
| Second devnet node | same host, `defcond-devnet2`, exists so the seed has a peer -- **and it stakes**: `staking=1`, wallet `devnet2`, 10.05 M DFCN, about 4 % of blocks (22 of 500, measured 2026-09-08). Nothing observes it directly -- the explorer's own `SeedStatusService` reads the seed's RPC, and since 2026-09-08 the **peer's wallet too**, through the optional `PEER_RPC_*` block. Its payout scripts are folded into the **`seed` host row**, because `byHost` groups production by machine and two daemons on one box are one machine. Until that block is set on a deployment, devnet2's key is unattributed -- and `byHost.hhi` is then `null` for the whole network, since the index is withheld while any producer is unmapped. The header still counts nine **stakers**: the eight fullnodes plus this one |
